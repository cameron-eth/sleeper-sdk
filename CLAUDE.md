...
The installable package lives in `python/` — that is the working directory for nearly every command. The root also holds `data/` (committed KTC snapshots), `.claude/commands/` (skills, see below), `.agents/skills/` (a **generated** Codex mirror — never edit it by hand), `scripts/` (repo tooling, not packaged), and two legacy standalone files (`sleeper_wrapper.py`, `sleeper-api.ts`) that are **not** part of the package and are not imported by it.

`.claude/STRUCTURE.md` is the deeper architecture doc for the skill and CLI layers; read it before adding either.
...
### The authenticated agent layer

Roughly half the CLI (23 of 38 commands) lives in `cli_agent.py` + `agent/helpers.py` and talks to Sleeper's **private GraphQL API** at `sleeper.com`, not the public read API. It needs `SLEEPER_TOKEN`. `auth/client.py` wraps 12 mutations: propose/accept/reject/cancel trade, set starters, add/drop, waiver claim + cancel, taxi, IR, activate.

**Every write goes through preview/execute** (`agent/preview.py`). The command builds the exact payload, prints it with a `preview_id`, and caches it to `~/.sleeper-sdk/previews/` with a 10-minute TTL. Nothing is sent until `--execute` or `sleeper execute <id>`. The disk cache is deliberate — it lets a separate process (a bot callback, a queued job) execute by id without re-fetching state. Preserve this pattern when adding a write; do not add a command that fires directly.

There is **no messaging or league-settings capability** — no `create_message`, no `update_league`. Commissioner-style automation (DM updates, changing league settings) would need new GraphQL discovery first, not just a new skill.
...
`.claude/commands/**/*.md` define 29 slash-command skills over the CLI, grouped into namespaces by subdirectory — `/roster:lineup`, `/trades:find`, `/market:value`, `/league:status`, `/strategy:gm-mode`.

Rules that are enforced by CI (`.github/workflows/skills.yml`), not by convention:

- **Every skill needs a real `description` in frontmatter.** Without one the harness falls back to the filename and the skill reaches the model as `gm-mode: gm-mode` — still loaded, but invisible to routing. This is silent, which is why it is gated.
- **`.claude/commands/` is the only source of truth.** `.agents/skills/` is generated by `scripts/sync_skills.py`; CI fails if it is stale. Edit the command, then run the script.
- **No skill may reference a CLI command that does not exist.**
- **Write skills carry `disable-model-invocation: true`** — `roster:set-lineup`, `roster:moves`, `roster:slots`, `trades:respond`, plus `roster:draft` (unbounded polling loop). They mutate a real league, so they are user-invoked only; "should I drop him?" is a question, not an instruction.
- **Skills take the user and league as arguments.** No skill hardcodes a username, league name or league ID.
